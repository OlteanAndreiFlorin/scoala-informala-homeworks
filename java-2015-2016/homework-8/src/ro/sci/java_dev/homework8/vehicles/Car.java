...
/**
 * Car is the abstract bases class for all cars created. It implements the
 * Vehicle() interface.
 * 
...
public abstract class Car implements Vehicle {
	...
	/**
	 * Overwrites the vehicle() implementation implementing the start() method
	 * for the Car() object; saves the fuel consumed and pollution created when
	 * "starting a car"; invokes the System.exit(); method when the car is
	 * started while running or when the car is started in a >1'st gear ;
	 */
	@Override
	public void start() throws RuntimeException {
		...
		if (this.currentGear > 1) {
			throw new RuntimeException(
					"Nice you broke the car :|,who starts their car in " + this.currentGear + "nd/rd gear");
	...
	/**
	 * Calculates the fuel consumed in a certain trip length; Invokes
	 * System.exit() if the car remains with no fuel Saves the fuel consumed and
	 * pollution for the given length;
	 */
	@Override
	...
	private void manadgeDriveModificationsToFields(double tripLength, float fuelConsumed) {// Manages
																							// the
																							// states
																							// of
																							// the
	...
	/**
	 * Saves consumption and pollution; Prints out a message confirming the
	 * invoking of the method sets the car status;
	 */
	@Override
	public void stop() {
	...
	/**
	 * Shifts the gear saving the new value in the same field invokes
	 * System.exit() if the gear dose not exist
	 * 
	 * @param gear
	 *            current gear
	 */
	public void shiftGear(int gear) throws RuntimeException {
		...
		if (gear > maxNumberOfGears) {
			throw new RuntimeException(
					"Nice you broke the car :| the car only has " + maxNumberOfGears + " gear/s dude");
		}
		setGear(gear);
	...
	private void saveStats(float fuelConsumed, float pollution) {// Saves the
																	// fuel and
																	// pollutuin
	...
	private void resetStats() {// resets the fuel consumed and pollution when
								// the car is started
	...
	public float calculateAverageFuelConsumption() throws RuntimeException {// calculates
																			// the
																			// avarage
																			// fuel
																			// consumption
